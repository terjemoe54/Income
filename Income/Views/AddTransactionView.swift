...
struct AddTransactionView: View {
    ...
    @AppStorage("currency") var currency = Currency.nok
    ...
    var body: some View {
        VStack {
            ...
            Picker("Choose State", selection: $selectedTransactionState) {
                ForEach(TransactionState.allCases) { transactionState in
                    Text(transactionState.state)
            ...
            Button(action: {
                ...
                if let transactionToEdit = transactionToEdit {
                    ...
                    let transaction = Transaction(title: transactionTitle, type: selectedTransactionType, state: selectedTransactionState, amount: amount, date: transactionToEdit.date)
                    transactions[indexOfTransaction] = transaction
                } else {
                    let transaction = Transaction(title: transactionTitle, type: selectedTransactionType, state: selectedTransactionState, amount: amount, date: Date())
        ...
        .onAppear(perform: {
            if let transactionToEdit = transactionToEdit {
                ...
                selectedTransactionState = transactionToEdit.state
            }
        })
        .padding(.top)
