...
struct HomeView: View {
    ...
    private var expenses: String {
        ...
        return numberFormatter.string(from: sumExpenses as NSNumber) ?? "NOK0.00"
    }
    
    private var income: String {
        let sumIncome = transactions.filter({ $0.type == .income }).reduce(0, { $0 + $1.amount })
        return numberFormatter.string(from: sumIncome as NSNumber) ?? "NOK0.00"
    }
    ...
    private var total: String {
        ...
        return numberFormatter.string(from: total as NSNumber) ?? "NOK0.00"
    }
